...
from src.visualization.visualize_adaptive_gamma_results import plot_dual_axis
...
def train_agent(strategy_name: str, agent_type: str = "dqn"):
    print(f"\n Starting Experiment: {strategy_name} | Agent: {agent_type.upper()}")

    run_name = f"{agent_type}_strategy_{strategy_name}"
    ...
    np.save(os.path.join(DATA_DIR, f"results_{agent_type}_{strategy_name}.npy"), history)
    
    if agent_type.lower() == "dqn":
        model_path = os.path.join(MODELS_DIR, f"{agent_type}_{strategy_name}.pth")
        torch.save(agent.policy_net.state_dict(), model_path)
    else:
        model_path = os.path.join(MODELS_DIR, f"{agent_type}_{strategy_name}.npy")
        np.save(model_path, agent.q_table)
    
    print(f" Completed {agent_type.upper()} | {strategy_name}")
